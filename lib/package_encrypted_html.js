...
function package_encrypted_content(dir, source_content, password) {
  ...
  const encoded_blod_content = to_encoded_text(dir, spec_blod_text, password);
  return encoded_blod_content;
}

// 加密指定文本，并塞入到验证div上，更新解密后被替换的div id
function to_encoded_text(dir, spec_blod_text, password) {
  ...
  const encoded_blod_content = encrypted_template_content.replace(/base64_text/g, encoded_content);
  return encoded_blod_content;
}
